...
def inicio(request: Request): # Funcion inicio para la página principal (nombre semantico) 
    mis_datos = {
        'nombre' : 'Proyecto de deteccion de nivel del Mar',
        'hobbie' : 'Este es proyecto se basa en:'
    }
...
@app.get("/prueba") #Configuro la ruta prueba
def inicio(request: Request): # Funcion inicio para la página principal (nombre semantico) 
    mis_datos = {
        'nombre' : 'Steven',
        'hobbie' : 'JOder las bolas'
    }
    return templates.TemplateResponse("prueba.html", {"request": request, "mis_datos": mis_datos}) #Devuelve un dato en forma de diccionario
